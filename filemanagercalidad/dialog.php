...
if (isset($_POST['submit'])) {
	...
	$rfm_subfolder = '';
	if (
		!empty($_SESSION['RF']["subfolder"]) && strpos($_SESSION['RF']["subfolder"], '../') === FALSE
	...
	if (isset($_GET['popup'])) $popup = strip_tags($_GET['popup']);
	else $popup = 0;
	//Sanitize popup
	$popup = !!$popup;
...
?>
	...
	<body>
		...
		<input type="hidden" id="fldr_value" value="<?php echo $_GET['car'] . "/" . $subdir; ?>" />
		...
		<input type="hidden" id="copy_cut_files_allowed" value="<?php if ($copy_cut_files) echo 1;
																else echo 0; ?>" />
		<input type="hidden" id="copy_cut_dirs_allowed" value="<?php if ($copy_cut_dirs) echo 1;
																else echo 0; ?>" />
		...
		<div class="container-fluid">
			...
			<div class="navbar navbar-fixed-top">
						...
						<div class="nav-collapse collapse" style="height:auto !important;">
							<div class="filters">
								<div class="row-fluid">
									...
									<div class="span3 " style="width:50% !important;float:left;">
			    ...
			    <?php } ?>-->
									...
									<div class="span3  view-controller" style="width:50% !important;float:left; text-align:right; margin:0 !important;">
			...
			<div class="row-fluid ff-container">
				<div class="span12">
					<?php if (@opendir($current_path . $rfm_subfolder . $subdir) === FALSE) { ?>
						<br />
						<div class="alert alert-error">Opps. Parece haber un error con el direcorio del funcionario </div>
					<?php } else { ?>
							...
							?>
								<li data-name="<?php echo $file ?>" <?php if ($file == '..') echo 'class="back"';
									...
									<?php } else { ?>
										...
										<figcaption>
											<a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($rename_folders && !$file_prevent_rename) echo "rename-folder"; ?>" title="<?php echo lang_Rename ?>" data-path="<?php echo $_GET['car'] . '/' . $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
												<i class="icon-pencil <?php if (!$rename_folders || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>
											<a href="javascript:void('')" class="tip-left erase-button <?php if ($delete_folders && !$file_prevent_delete) echo "delete-folder"; ?>" title="<?php echo lang_Erase ?>" data-confirm="<?php echo lang_Confirm_Folder_del; ?>" data-path="<?php echo $_GET['car'] . '/' . $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
							...
							foreach ($files as $nu => $file_array) {
								...
								?>
									<li class="ff-item-type-<?php echo $class_ext; ?> file" data-name="<?php echo $file; ?>"><?php
											...
											<a href="<?php echo $current_path . $subdir . $file; ?>" target="_blank" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $field_id; ?>" data-function="<?php echo $apply; ?>">
												<div class="img-precontainer">
											...
											<figcaption>
												<form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
													<input type="hidden" name="path" value="<?php echo $_GET['car'] . '/' . $rfm_subfolder . $subdir ?>" />
													...
													<?php } elseif (($is_video || $is_audio) && in_array($extension_lower, $jplayer_ext)) { ?>
														<a class="tip-right modalAV <?php if ($is_audio) {
																						...
																						echo "video";
																					} ?>" title="<?php echo lang_Preview ?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $current_path . $rfm_subfolder . $subdir . $file;; ?>" href="javascript:void('');"><i class=" icon-eye-open"></i></a>
													<?php } else { ?>
														<a class="preview disabled"><i class="icon-eye-open icon-white"></i></a>
													<?php } ?>
													<a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($rename_files && !$file_prevent_rename) echo "rename-file"; ?>" title="<?php echo lang_Rename ?>" data-path="<?php echo $_GET['car'] . '/' . $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
														<i class="icon-pencil <?php if (!$rename_files || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>

													<a href="javascript:void('')" class="tip-left erase-button <?php if ($delete_files && !$file_prevent_delete) echo "delete-file"; ?>" title="<?php echo lang_Erase ?>" data-confirm="<?php echo lang_Confirm_del; ?>" data-path="<?php echo $_GET['car'] . '/' . $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
